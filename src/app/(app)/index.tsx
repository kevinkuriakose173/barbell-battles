...
export default function HomeScreen() {
  ...
  const loadLifts = useCallback(async () => {
    try {
      setExercises(await getExercisesWithEntries());
    } catch (error) {
      Alert.alert('Unable to load lifts', error instanceof Error ? error.message : 'Try again.');
    } finally {
      setIsLoading(false);
    }
  }, []);
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.headerRow}>
          ...
          <Pressable onPress={handleSignOut} hitSlop={12}><Text style={styles.signOut}>Sign out</Text></Pressable>
        </View>
